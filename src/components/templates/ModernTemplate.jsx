...
export default function ModernTemplate({ state, themeColor, fontSize, fontFamily, spacing }) {
          ...
          <div className="tmpl-section" style={{ marginBottom: spacing }} key="skills">
            ...
            {skills.textContent ? (
              ...
              <div className="tmpl-skills-grid">
                {skills.ratings.map(s => <span key={s.id} className="tmpl-skill-pill" style={{ backgroundColor: themeColor, color: 'white' }}>{s.name}</span>)}
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className="template-modern" style={{ fontFamily, fontSize, color: '#334155' }}>
      <header className="modern-header" style={{ display: 'flex', justifyContent: 'space-between', borderTop: `6px solid ${themeColor}`, padding: '32px 32px 16px 32px' }}>
      ...
      <div className="modern-body" style={{ padding: '0 32px 32px 32px' }}>
