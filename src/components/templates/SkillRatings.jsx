...
export default function SkillRatings({ ratings = [], showRatings = true }) {
  const visibleRatings = (Array.isArray(ratings) ? ratings : []).filter(skill => skill?.name?.trim());
  if (!visibleRatings.length) return null;

  return (
    <ul className={`tmpl-skill-ratings${showRatings ? '' : ' tmpl-skill-names-only'}`} aria-label={showRatings ? 'Skill ratings' : 'Skills'}>
